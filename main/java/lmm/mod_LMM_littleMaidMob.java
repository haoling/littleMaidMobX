	...
	public void modsLoaded() {
		...
		if(cfg_spawnWeight > 0) {
			if (cfg_Dominant) {
				// あらゆる場所にスポーンする
				try {
					Field afield[] = (lmm.BiomeGenBase.class).getDeclaredFields();
					LinkedList<BiomeGenBase> linkedlist = new LinkedList<BiomeGenBase>();
					for(int j = 0; j < afield.length; j++) {
						Class class1 = afield[j].getType();
						if((afield[j].getModifiers() & 8) != 0 && class1.isAssignableFrom(lmm.BiomeGenBase.class)) {
					...
					ModLoader.addSpawn(lmm.LMM_EntityLittleMaid.class, cfg_spawnWeight, cfg_minGroupSize, cfg_maxGroupSize, EnumCreatureType.creature, dominateBiomes);
